...
class MassSpecFormer(nn.Module):
    def __init__(self, input_dim=2, hidden_dim=128, num_layers=6, num_classes=56):  # ✅ Updated for 56 antibiotics
        ...
        self.fc = nn.Linear(hidden_dim, num_classes)  # ✅ Updated for 56 antibiotics
...
# -------------------------------
# 3️⃣ Load Pretrained Model & Modify Last Layer
# -------------------------------
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
print(f"🔥 Using device: {device}")

num_antibiotics = 56  # ✅ Adjust model to match dataset
model = MassSpecFormer(num_classes=num_antibiotics).to(device)

# 🔹 Load pretrained weights but ignore the final classification layer
pretrained_dict = torch.load("./trained_models/massspecformer.pth")
model_dict = model.state_dict()

# Remove the final fully connected (fc) layer from the pre-trained weights
